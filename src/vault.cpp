...
public:
    ProgressIndicator(const std::string& operation, uint64_t totalSize)
        : operation_(operation), totalSize_(totalSize), currentSize_(0), lastPercent_(-1) {
        if (totalSize_ > PROGRESS_THRESHOLD) {
            showProgress_ = true;
    ...
    void update(uint64_t bytesProcessed) {
        if (!showProgress_) return;

        currentSize_ = bytesProcessed;
        int percent = static_cast<int>((currentSize_ * 100) / totalSize_);

        if (percent != lastPercent_ && percent % 10 == 0) {
...
private:
    static constexpr uint64_t PROGRESS_THRESHOLD = 5 * 1024 * 1024; // 5 MB
...
std::unique_ptr<Vault> Vault::open(const std::filesystem::path& vaultPath,
    ...
    std::vector<uint8_t> nonce(configFile.begin() + SALT_SIZE, configFile.begin() + SALT_SIZE + NONCE_SIZE);
    std::vector<uint8_t> encryptedConfig(configFile.begin() + SALT_SIZE + NONCE_SIZE, configFile.end());
...
std::string Vault::addFile(const std::filesystem::path& filePath) {
    ...
    uint64_t fileSize = FileOperations::getFileSize(filePath);

    // Read file
    std::vector<uint8_t> fileData;
    {
        ProgressIndicator progress("Reading file", fileSize);
    ...
    {
        ProgressIndicator progress("Encrypting", fileSize);
...
bool Vault::extractFile(const std::string& fileId,
    ...
    // Get file metadata
    const auto* metadata = metadata_.getFile(fileId);
    if (!metadata) {
    ...
    std::vector<uint8_t> encryptedData;
    {
        ProgressIndicator progress("Reading from vault", metadata->encryptedSize);
        if (!FileOperations::readFile(encryptedPath, encryptedData)) {
            std::cerr << "Error: Failed to read encrypted file\n";
            return false;
        }
        progress.update(metadata->encryptedSize);
    }

    // Decrypt file
    std::vector<uint8_t> fileData;
    {
        ProgressIndicator progress("Decrypting", metadata->originalSize);
        if (!Crypto::decrypt(encryptedData, key_, metadata->nonce, fileData)) {
            std::cerr << "Error: Failed to decrypt file\n";
            return false;
        }
        progress.update(metadata->originalSize);
    }
    ...
    {
        ProgressIndicator progress("Writing file", fileData.size());
        if (!FileOperations::writeFile(outputPath, fileData)) {
            std::cerr << "Error: Failed to write output file\n";
            return false;
        }
        progress.update(fileData.size());
    }
